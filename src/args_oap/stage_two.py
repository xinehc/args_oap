...
class StageTwo:
    def __init__(self, options):
        ...
        for file in (
            glob.glob(os.path.join(self.outdir, 'absolute_concentration.*.txt')) +
            glob.glob(os.path.join(self.outdir, 'absolute_quantification.*.txt'))
        ):
            os.remove(file)
        ...
        ## optional physical metadata generated by stage_one
        self.absolute_metadata = pd.DataFrame()
        for file in [
            self.setting.metadata_with_absolute_quantification,
            os.path.join(self.outdir, 'metadata_addabs.txt')
        ]:
            try:
                os.remove(file)
            except OSError:
                pass
        if os.path.isfile(self.setting.absolute_metadata):
            absolute_input = pd.read_table(self.setting.absolute_metadata, dtype={'sample': str})
            absolute_input = absolute_input.dropna()
            if len(absolute_input) > 0:
                sequenced_dna_g = (
                    absolute_input['An'] * 313.2 + absolute_input['Tn'] * 304.2 +
                    absolute_input['Cn'] * 289.2 + absolute_input['Gn'] * 329.2
                ) / 6.02e23 * 2
                extracted_dna_g = (
                    absolute_input['ElutionVolume_uL'] * absolute_input['DNAConcentration_ng/uL'] * 1e-9
                )
                absolute_input['dna_mass_ratio'] = extracted_dna_g / sequenced_dna_g
                self.absolute_metadata = pd.merge(self.metadata, absolute_input, on='sample')
                self.absolute_metadata['cell_concentration'] = (
                    self.absolute_metadata['nCell'] / self.absolute_metadata['SampleVolume_mL'] *
                    self.absolute_metadata['dna_mass_ratio']
                )
    ...
    def merge_files(self):
        ...
        ## calculate total ARG copies/mL and add them to the sample metadata
        absolute_out = pd.DataFrame()
        if len(self.absolute_metadata) > 0:
            absolute_out = pd.merge(self.absolute_metadata, result, on='sample')
            abundance_arg_copies_per_cell = absolute_out['copy'] / absolute_out['nCell']
            absolute_out['value'] = abundance_arg_copies_per_cell * absolute_out['cell_concentration']
            absolute_total = absolute_out.groupby('sample', as_index=False)['value'].sum().rename(
                columns={'value': 'absolute_quantification'})
            pd.merge(
                self.metadata, absolute_total, on='sample', how='inner'
            ).to_csv(self.setting.metadata_with_absolute_quantification, sep='\t', index=False)
        ...
        for level in levels:
            ...
            ## absolute gene concentration in the original sample (copies/mL)
            if len(absolute_out) > 0:
                out_agg = absolute_out.groupby([level, 'sample'])['value'].sum().unstack().fillna(0)
                out_agg.sort_index().to_csv(os.path.join(self.outdir, f'absolute_quantification.{level}.txt'), sep='\t')
